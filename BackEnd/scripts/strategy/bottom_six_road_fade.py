#!/usr/bin/env python3
import argparse
import json
import sys
from app.db import get_connection

def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("--season", type=int, required=True)
    parser.add_argument("--contest-format", choices=["STANDARD", "CIRCA"], required=True)
    parser.add_argument("--rating-week", type=int, required=True)
    parser.add_argument("--hfa-source", required=True)
    return parser.parse_args()


def get_bottom_six_teams(cur, season, week):
    """
    Identifies the six worst teams in the league based on their 
    average PFF Point Spread Point/Power Ratings for the given slate snapshot.
    """
    cur.execute("""
        SELECT team_id, pff_team_code
        FROM ratings.pff_power_ratings
        WHERE season = %s AND week = %s
        ORDER BY point_spread_rating ASC
        LIMIT 6;
    """, (season, week))
    return {row[0]: row[1] for row in cur.fetchall()}


def get_road_fade_candidates(cur, args, leg, bottom_six_ids):
    """
    Queries matchups where a designated bottom-six team is playing on the road,
    joined with the V3 risk calibration metrics for the expected favorite.
    """
    if leg["special_leg_type"] == "THANKSGIVING":
        filter_sql = "g.is_thanksgiving = TRUE"
        params = [args.season, args.rating_week, args.hfa_source]
    elif leg["special_leg_type"] == "CHRISTMAS":
        filter_sql = "g.is_christmas = TRUE"
        params = [args.season, args.rating_week, args.hfa_source]
    else:
        filter_sql = "g.week = %s"
        params = [args.season, args.rating_week, args.hfa_source, leg["nfl_week"]]
        if args.contest_format == "CIRCA":
            filter_sql += " AND g.is_thanksgiving = FALSE AND g.is_christmas = FALSE"

    # We join with the bottom six IDs explicitly tracking away_team_id
    bottom_six_csv = ",".join(map(str, bottom_six_ids)) if bottom_six_ids else "-1"

    cur.execute(f"""
        SELECT
            p.game_id,
            p.projected_favorite_team_id AS team_id,
            p.projected_favorite_abbr,
            p.projected_spread,
            ABS(p.projected_spread) AS spread_strength,
            g.away_team_id,
            g.home_team_id,
            COALESCE(r.risk_score, 0.0) AS risk_score,
            COALESCE(r.risk_stars, 1) AS risk_stars,
            COALESCE(r.risk_level, 'LOW') AS risk_level,
            COALESCE(r.risk_summary, 'No significant risk factors detected.') AS risk_summary,
            -- Sorting logic: prioritize matching against the road fade target with lowest risk scores
            (ABS(p.projected_spread) - COALESCE(r.risk_score, 0.0)) AS strategy_score
        FROM projections.game_spreads p
        JOIN schedule.games g ON g.game_id = p.game_id
        LEFT JOIN risk.game_risk_scores r ON r.game_id = p.game_id AND r.team_id = p.projected_favorite_team_id
        WHERE p.season = %s
          AND p.rating_week = %s
          AND p.hfa_source_system = %s
          AND {filter_sql}
          AND g.away_team_id IN ({bottom_six_csv})
        ORDER BY (ABS(p.projected_spread) - COALESCE(r.risk_score, 0.0)) DESC;
    """, params)

    cols = [d[0] for d in cur.description]
    return [dict(zip(cols, row)) for row in cur.fetchall()]


def get_backup_favorites(cur, args, leg):
    """
    Fallback method: Fetches standard safest favorites if no bottom-six teams 
    are playing on the road during a specific contest leg.
    """
    if leg["special_leg_type"] == "THANKSGIVING":
        filter_sql = "g.is_thanksgiving = TRUE"
        params = [args.season, args.rating_week, args.hfa_source]
    elif leg["special_leg_type"] == "CHRISTMAS":
        filter_sql = "g.is_christmas = TRUE"
        params = [args.season, args.rating_week, args.hfa_source]
    else:
        filter_sql = "g.week = %s"
        params = [args.season, args.rating_week, args.hfa_source, leg["nfl_week"]]
        if args.contest_format == "CIRCA":
            filter_sql += " AND g.is_thanksgiving = FALSE AND g.is_christmas = FALSE"

    cur.execute(f"""
        SELECT
            p.game_id,
            p.projected_favorite_team_id AS team_id,
            p.projected_favorite_abbr,
            p.projected_spread,
            ABS(p.projected_spread) AS spread_strength,
            COALESCE(r.risk_score, 0.0) AS risk_score,
            COALESCE(r.risk_stars, 1) AS risk_stars,
            COALESCE(r.risk_level, 'LOW') AS risk_level,
            COALESCE(r.risk_summary, 'No significant risk factors detected.') AS risk_summary,
            ABS(p.projected_spread) AS strategy_score
        FROM projections.game_spreads p
        JOIN schedule.games g ON g.game_id = p.game_id
        LEFT JOIN risk.game_risk_scores r ON r.game_id = p.game_id AND r.team_id = p.projected_favorite_team_id
        WHERE p.season = %s
          AND p.rating_week = %s
          AND p.hfa_source_system = %s
          AND {filter_sql}
        ORDER BY ABS(p.projected_spread) DESC;
    """, params)

    cols = [d[0] for d in cur.description]
    return [dict(zip(cols, row)) for row in cur.fetchall()]


def get_current_application_context(cur):
    cur.execute("""
        SELECT current_week 
        FROM system.application_context 
        WHERE is_active = TRUE 
        LIMIT 1;
    """)
    row = cur.fetchone()
    return row[0] if row else 1


def main():
    args = parse_args()
    output = {
        "strategy": "BOTTOM_SIX_ROAD_FADE",
        "strategy_version": "v3.0",
        "season": args.season,
        "contest_format": args.contest_format,
        "rating_week": args.rating_week,
        "hfa_source": args.hfa_source,
        "entries": []
    }

    with get_connection() as conn:
        with conn.cursor() as cur:
            current_active_week = get_current_application_context(cur)
            bottom_six = get_bottom_six_teams(cur, args.season, args.rating_week)
            bottom_six_ids = list(bottom_six.keys())

            cur.execute("""
                SELECT entry_id, survivor_sweat_name
                FROM survivor.entries
                WHERE is_active = TRUE
                ORDER BY entry_id;
            """)
            entries = cur.fetchall()

            cur.execute("""
                SELECT
                    l.contest_leg_id,
                    l.leg_number,
                    l.leg_code,
                    l.leg_name,
                    l.nfl_week,
                    l.special_leg_type
                FROM contest.legs l
                JOIN contest.formats f ON f.contest_format_id = l.contest_format_id
                WHERE l.season = %s
                  AND f.format_code = %s
                ORDER BY l.leg_number;
            """, (args.season, args.contest_format))

            cols = [d[0] for d in cur.description]
            legs = [dict(zip(cols, row)) for row in cur.fetchall()]

            if args.contest_format == "CIRCA":
                special = [l for l in legs if l["special_leg_type"] in ("THANKSGIVING", "CHRISTMAS")]
                normal = [l for l in legs if l["special_leg_type"] is None]
                processing_order = special + normal
            else:
                processing_order = legs

            for entry_id, sweat_name in entries:
                cur.execute("SELECT team_id FROM survivor.entry_picks WHERE entry_id = %s;", (entry_id,))
                used = {r[0] for r in cur.fetchall()}

                picks = []
                current_week_alternatives = None

                for leg in processing_order:
                    candidates = get_road_fade_candidates(cur, args, leg, bottom_six_ids)
                    is_fallback_used = False

                    # If no road targets match this specific slot, fetch baseline safe pool options
                    if not candidates:
                        candidates = get_backup_favorites(cur, args, leg)
                        is_fallback_used = True

                    chosen = None
                    valid_idx = -1
                    for idx, c in enumerate(candidates):
                        if c["team_id"] not in used:
                            chosen = c
                            valid_idx = idx
                            break

                    if chosen:
                        used.add(chosen["team_id"])
                        
                        rationale_str = (
                            "Targeted matchup opposing a bottom-six team on the road."
                            if not is_fallback_used else
                            "Fallback assignment: No bottom-six teams on the road; defaulted to safest overall favorite."
                        )

                        picks.append({
                            "leg_number": leg["leg_number"],
                            "leg_code": leg["leg_code"],
                            "leg_name": leg["leg_name"],
                            "team": chosen["projected_favorite_abbr"],
                            "projected_line": f"{chosen['projected_favorite_abbr']} {round(float(chosen['projected_spread']), 1)}",
                            "game_id": chosen["game_id"],
                            "risk_stars": chosen["risk_stars"],
                            "risk_level": chosen["risk_level"],
                            "risk_points": float(chosen["risk_score"]),
                            "risk_summary": chosen["risk_summary"],
                            "rationale": rationale_str
                        })

                        # Compile alternative options for active week view panel
                        if leg["nfl_week"] == current_active_week and leg["special_leg_type"] is None:
                            alt_nodes = []
                            for alt_c in candidates[valid_idx + 1:]:
                                if alt_c["team_id"] not in used and alt_c["game_id"] != chosen["game_id"]:
                                    alt_nodes.append({
                                        "team": alt_c["projected_favorite_abbr"],
                                        "projected_line": f"{alt_c['projected_favorite_abbr']} {round(float(alt_c['projected_spread']), 1)}",
                                        "game_id": alt_c["game_id"],
                                        "risk_stars": alt_c["risk_stars"],
                                        "risk_level": alt_c["risk_level"],
                                        "risk_points": float(alt_c["risk_score"]),
                                        "risk_summary": alt_c["risk_summary"]
                                    })
                                    if len(alt_nodes) >= 2:
                                        break
                            current_week_alternatives = alt_nodes

                picks.sort(key=lambda x: x["leg_number"])

                output["entries"].append({
                    "entry_id": entry_id,
                    "survivor_sweat_name": sweat_name,
                    "alternative_recommendations": current_week_alternatives or [],
                    "picks": picks
                })

    print(json.dumps(output, indent=2, default=str))


if __name__ == "__main__":
    main()