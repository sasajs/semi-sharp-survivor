...
test(
    "Strategy Current Week Highest Win",
    run_script(
        "Current Week Highest Win",
        """
        PYTHONPATH=. python3 scripts/strategy/current_week_highest_win.py \
        --season 2026 \
...
test(
    "Strategy Bottom Six Road Fade",
    run_script(
        "Bottom Six Road Fade",
        """
        PYTHONPATH=. python3 scripts/strategy/bottom_six_road_fade.py \
        --season 2026 \
        --contest-format STANDARD \
        --rating-week 1 \
        --hfa-source SEMISHARP_2026 > /dev/null
...
test(
    "Strategy Market Arbitrage Exit",
    run_script(
        "Market Arbitrage Exit",
        """
        PYTHONPATH=. python3 scripts/strategy/market_arbitrage_exit.py \
        --season 2026 \
        --contest-format STANDARD \
        --rating-week 1 \
        --hfa-source SEMISHARP_2026 > /dev/null
...
test(
    ...
    api_test(
        "/strategies",
        lambda x: assert_minimum(len(x), 8)  # Changed from x["count"] to len(x) for JSON array
    )
)
...
test(
    "API Strategy Current Week Highest Win",
    api_test(
        "/strategies/current-week-highest-win/2026/STANDARD"
    )
)
...
test(
    "API Strategy Bottom Six Road Fade",
    api_test(
        "/strategies/bottom-six-road-fade/2026/STANDARD?rating_week=1&hfa_source=SEMISHARP_2026"
    )
)

test(
    "API Strategy Market Arbitrage Exit",
    api_test(
        "/strategies/market-arbitrage-exit/2026/STANDARD?rating_week=1&hfa_source=SEMISHARP_2026"
    )
)
