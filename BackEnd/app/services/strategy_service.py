...
def current_week_highest_win(season, contest_format, rating_week=1, hfa_source="SEMISHARP_2026", entry_id=1):
...
def future_value(season, contest_format, rating_week=1, hfa_source="SEMISHARP_2026", entry_id=1):
    # Fixed: Restored required --rating-week and --hfa-source arguments
    return run_strategy("scripts/strategy/future_value.py", [
        "--season", str(season), "--contest-format", contest_format, 
        "--rating-week", str(rating_week), "--hfa-source", hfa_source, "--entry-id", str(entry_id)
    ])

def multiple_entry(season, contest_format, rating_week=1, hfa_source="SEMISHARP_2026", user_id=1):
...
def monte_carlo(season, contest_format, rating_week=1, hfa_source="SEMISHARP_2026"):
    return run_strategy("scripts/strategy/monte_carlo_survivor.py", [
        "--season", str(season), "--contest-format", contest_format,
        "--rating-week", str(rating_week), "--hfa-source", hfa_source
    ])

def dynamic_programming(season, contest_format, rating_week=1, hfa_source="SEMISHARP_2026"):
    # Fixed: Restored required --rating-week and --hfa-source arguments
...
def bottom_six_road_fade(season, contest_format, rating_week=1, hfa_source="SEMISHARP_2026"):
    return run_strategy("scripts/strategy/bottom_six_road_fade.py", [
        "--season", str(season), "--contest-format", contest_format,
        "--rating-week", str(rating_week), "--hfa-source", hfa_source
    ])

def market_arbitrage_exit(season, contest_format, rating_week=1, hfa_source="SEMISHARP_2026"):
